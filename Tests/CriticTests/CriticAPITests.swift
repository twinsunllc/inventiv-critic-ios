...
final class MockURLProtocol: URLProtocol, @unchecked Sendable {
    ...
    /// Handler that provides (response, data) for each request.
    nonisolated(unsafe) static var requestHandler: ((URLRequest) throws -> (HTTPURLResponse, Data))?

    /// Captured requests for assertion.
    nonisolated(unsafe) static var capturedRequests: [URLRequest] = []

    /// Captured request bodies (httpBody may be nil after URLProtocol round-trip).
    ...
    override func startLoading() {
        ...
        // Capture body from httpBody or from the input stream
...
// MARK: - API Tests (serialized to avoid shared mock state races)

@Suite(.serialized)
struct CriticAPINetworkTests {
    ...
    @Test func pingRequestBodyContainsCorrectFields() async throws {
        ...
        let bodyData = MockURLProtocol.capturedBodies.first ?? Data()
    ...
    @Test func createBugReportSendsMultipartPOST() async throws {
        ...
        let bodyString = String(data: MockURLProtocol.capturedBodies.first ?? Data(), encoding: .utf8) ?? ""
